...
/** Driver for the Bma42x accelerometer sensor.
 * Original implementation from wasp-os : https://github.com/daniel-thompson/wasp-os/blob/master/wasp/drivers/Bma42x.py
 */
Bma42x::Bma42x(TwiMaster &twiMaster, uint8_t twiAddress) : twiMaster{twiMaster}, twiAddress{twiAddress} {
...
void Bma42x::Init() {
  ...
  // and is used by at least two other Bma42x drivers
...
void Bma42x::SetGain(uint8_t gain) {
...
void Bma42x::SetDrive(uint8_t drive) {
...
void Bma42x::WriteRegister(uint8_t reg, uint8_t data) {
  auto ret = twiMaster.Write(twiAddress, reg, &data, 1);
  if(ret != TwiMaster::ErrorCodes::NoError)
    NRF_LOG_INFO("WRITE ERROR");
}

uint8_t Bma42x::ReadRegister(uint8_t reg) {
  uint8_t value;
